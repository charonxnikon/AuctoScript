...
struct market situation(int lvl, int active)
{
   struct market shop; 
   switch (lvl)
   {
...
exec_prod(struct info_ *tmp, struct request cmd)
{
   ...
   static const char a3[] = "Nikon: >> NOT ENOUGH FACTORIES\n "; 
   static const char a4[] = "Nikon: >> Factory accept\n"; 
   int count = tmp->order+cmd.op1;
   if (tmp->money-2000*count <= 0){
      send_msg(tmp, a1);
      return;
   }
...
void 
exec_buy(struct info_ *tmp, struct request cmd, int lvl, int active)
{
...
void
exec_sell(struct info_ *tmp, struct request cmd, int lvl, int active)
{
...
void
auction(struct info_ *data, int lvl)
{
   int i = 0; 
   int active = get_conline(data);
...
finance(struct info_ *data)
{
   ...
   while (tmp){
      ...
      pay += (tmp->materials)*300; 
      tmp->materials += tmp->bought; 
      pay += (tmp->bought)*(tmp->buy.price);
      pay += (tmp->factory)*1000;
      pay += build_factory(tmp);
      tmp->money -= pay;
      tmp->money += earn; 
      send_msg(tmp ,a1);
      send_cnt(tmp, earn);
...
struct info_ *
gameplay(struct info_ *data, struct game *bank)
{
   if (bank->play == off){
